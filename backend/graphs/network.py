...
class NodeNetwork:
    ...
    def initNodes(self, init_pops):
        ...
        for code_id in range(len(self.players)):
            ...
            self.nodes[0].append(next_node)

    def update(self, adj, pops):
        self.nodes.append(deepcopy(self.nodes[-1]))
        for i in range(len(self.nodes[-1])):
            self.nodes[-1][i].popularity = pops[i]
        for _ in range(self.steps):
            for curr_node in self.nodes[-1][::-1]:
                other_nodes = [node for node in self.nodes[-1] if node.code != curr_node.code]
                curr_node.update(other_nodes, adj)

    def backStep(self):
        self.nodes.pop()

    def _graphPopularities(self, ax, fig, pops_arr, type, color_lookup=None, name_lookup={}, legend_colors={}):
        rounds = len(pops_arr)
        ...
        else: ax.set_ylabel('Number of Chats', fontsize=12)
    ...
    def graphExchange(self, ax, fig, exchanges, step=-1, color_lookup=None, name_lookup={}):
        ...
        for p_i in self.nodes[-1]:
            for p_j in self.nodes[-1]:
                ...
                #adjacency matrix of who is talking to who (binary; 0 and 1)
