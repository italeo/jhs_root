...
with open(load_path) as json_file:
    ...
    alpha = data_dict["publicInfo"]["gameParams"]["popularityFunctionParams"]["alpha"]
    beta = data_dict["publicInfo"]["gameParams"]["popularityFunctionParams"]["beta"]
    c_g = data_dict["publicInfo"]["gameParams"]["popularityFunctionParams"]["cGive"]
    c_k = data_dict["publicInfo"]["gameParams"]["popularityFunctionParams"]["cKeep"]
    c_s = data_dict["publicInfo"]["gameParams"]["popularityFunctionParams"]["cSteal"]
    num_rounds = len(data_dict['privateInfo']['masterPopularities'])
    ...
    plrs = list(data_dict['privateInfo']['masterPopularities']["round_1"].keys())
    ...
    for r in range(num_rounds):
        ...
        for n, p in data_dict['privateInfo']['masterPopularities'][round_key].items():
            game_dict[f'p{players2code[n]}'].append(p)
            game_dict[f'pid{players2code[n]}'].append('Human')
   
        for receiver, infl_dict in data_dict['privateInfo']['masterInfluences'][round_key].items():
        ...
        if r != 0:
            for giver, action_dict in data_dict['privateInfo']['masterTransactions'][action_round_key].items():
                giver_id = players2code.get(giver, None)
