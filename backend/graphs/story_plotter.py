...
class StoryPlot():
    ...
    def plot_game(self, rounds):

        print('gamecode: ' + self.game_code)
        pids, chat_mat, chat_actions_mat, chat_infl_mat, chat_params = load_chat_data(self.game_code,self.folder)
        ...
        #ignore
        gs = GridSpec(4, len(rounds), figure=fig)
        ...
        for r in range(pop_mat.shape[0]):
            ...
            if r in rounds:
                ...
                ax.set_title(f'Tokens Round {r-1}', fontsize=12, loc='center', y=-0.09)
        ax = fig.add_subplot(gs[0, :])
        net._graphPopularities(ax, fig, pop_mat, type='pop', color_lookup=name2color, name_lookup=name2codename, legend_colors=legend_colors)
        fig.subplots_adjust(wspace=0.0, hspace=0.06)

        for r in range(chat_mat.shape[0]-1):
            chatNet.update(chat_infl_mat[r], chat_mat[r])
            if r in rounds:
                rightround = r+1
                round_idx = rounds.index(r)
                chatAx = fig.add_subplot(gs[2, ids[round_idx][1]], facecolor='c' if r%2 ==1 else 'm', ymargin=-.4)
                chatNet.graphExchange(chatAx, fig, chat_actions_mat[r-1], color_lookup=name2color, name_lookup=name2codename)
                chatAx.set_title(f'Chat Round {r-1}', fontsize=12, loc='center', y=-0.09)
        chatAx = fig.add_subplot(gs[3, :])
        chatNet._graphPopularities(chatAx, fig, chat_mat, type='chat', color_lookup=name2color, name_lookup=name2codename, legend_colors=legend_colors)
        fig.subplots_adjust(wspace=0.0, hspace=0.06)

        #plt.show()
        image_path = f'./images/storyplots/storyplot_{self.game_code}.png'
